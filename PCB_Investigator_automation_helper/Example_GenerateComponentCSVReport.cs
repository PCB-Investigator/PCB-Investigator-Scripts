...
/// </summary>
    ...
    {
        /// <summary>
        /// Example method to generate a CSV report of component details by using the PCB-Investigator API.
        /// </summary>
        private static string Example_GenerateComponentCSVReport(IPCBIWindow pcbi, IStep step, CancellationToken? cancelToken, string initialDir, string filter, int filterIndex, string fileName, string loadedDesignPathOrServer, PCBI.Plugin.Interfaces.FileLocation fileLocation, FileType fileType, string defaultFileName)
        {
            ...
            headerRow.Add(new PCBI.Automation.IO.Excel.Cell() { Value = "X Position (mm)", Type = PCBI.Automation.IO.Excel.Cell.CellType.STRING });
            headerRow.Add(new PCBI.Automation.IO.Excel.Cell() { Value = "Y Position (mm)", Type = PCBI.Automation.IO.Excel.Cell.CellType.STRING });
            ...
            // Iterate through all components to add their details to the CSV
            foreach (ICMPObject cmp in step.GetAllCMPObjects().OrderBy(c => c.Ref))
            {
                ...
                row.Add(new PCBI.Automation.IO.Excel.Cell() { Value = IMath.Mils2MM(positionMils.X), Type = PCBI.Automation.IO.Excel.Cell.CellType.DOUBLE });
                row.Add(new PCBI.Automation.IO.Excel.Cell() { Value = IMath.Mils2MM(positionMils.Y), Type = PCBI.Automation.IO.Excel.Cell.CellType.DOUBLE });
            ...
            // Show save file dialog to save the CSV file
            ...
            fileFilters.Add(new FileFormatFilter("CSV-Datei (*.csv)", ".csv"));
            fileFilters.Add(new FileFormatFilter("All files(*.*)", ""));
            saveFileDlg.InitDataProviders(loadedDesignPathOrServer: loadedDesignPathOrServer, fileLocation: fileLocation, fileType: fileType, defaultFileName, fileFilters: fileFilters);
        ...
        /// Example method to generate a CSV report of component details by using the PCB-Investigator API.
        /// </summary>
        private static string Example_GenerateComponentCSVReport(IPCBIWindow pcbi, IStep step, CancellationToken? cancelToken)
        {
            // Check if a job is loaded
            if (!pcbi.JobIsLoaded) return "No job is loaded.";
            ...
            // Show save file dialog to save the CSV file
            PCBISaveFileDlg saveFileDlg = new PCBISaveFileDlg(pcbi);
            saveFileDlg.InitBrowse(initialDir: "", filter: "CSV Files (*.csv)|*.csv", filterIndex: 0, fileName: "componentlist.csv");
            List<FileFormatFilter> fileFilters = new List<FileFormatFilter>();
            fileFilters.Add(new FileFormatFilter("CSV-Datei (*.csv)", ".csv"));
            fileFilters.Add(new FileFormatFilter("All files(*.*)", ""));
            saveFileDlg.InitDataProviders(loadedDesignPathOrServer: "", fileLocation: PCBI.Plugin.Interfaces.FileLocation.Global, fileType: FileType.TabularReport, "componentlist.csv", fileFilters: fileFilters);
            if (saveFileDlg.ShowDialog() != DialogResult.OK)
                return "The file save dialog has been canceled.";
        ...
        /// Example method to generate a CSV report of component details by using the PCB-Investigator API
        /// </summary>
        private static string Example_GenerateComponentCSVReport(IPCBIWindow pcbi, IStep step, CancellationToken? cancelToken, string initialDir, string fileName, string loadedDesignPathOrServer)
        {
            if (!pcbi.JobIsLoaded) return "No job is loaded.";
            List<List<PCBI.Automation.IO.Excel.Cell>> cellList = new List<List<PCBI.Automation.IO.Excel.Cell>>();
            List<PCBI.Automation.IO.Excel.Cell> headerRow = new List<PCBI.Automation.IO.Excel.Cell>
        ...
        private static string Example_GenerateComponentCSVReport(IPCBIWindow pcbi, IStep step, CancellationToken? cancelToken)
        {
            ...
            saveFileDlg.InitDataProviders(loadedDesignPathOrServer: "", fileLocation: PCBI.Plugin.Interfaces.FileLocation.Global,
                                          fileType: FileType.TabularReport, "componentlist.csv", fileFilters: fileFilters);
            if (saveFileDlg.ShowDialog() != DialogResult.OK)
                return "The file save dialog has been canceled.";
            IFileData fileData = saveFileDlg.GetFileData();
            if (fileData == null)
        ...
        /// <summary>
        /// Example method to generate a CSV report of component positions and details by using the PCB-Investigator API.
        /// </summary>
        private static string Example_GenerateComponentCSVReport(IPCBIWindow pcbi, IStep step, CancellationToken? cancelToken, string initialDir, string fileName)
        {
            if (!pcbi.JobIsLoaded) return "No job is loaded.";
            List<List<PCBI.Automation.IO.Excel.Cell>> cellList = new List<List<PCBI.Automation.IO.Excel.Cell>>();
            List<PCBI.Automation.IO.Excel.Cell> headerRow = new List<PCBI.Automation.IO.Excel.Cell>
        ...
        /// Example method to generate a CSV report of component positions and details by using the PCB-Investigator API.
        /// </summary>
        private static string Example_GenerateComponentCSVReport(IPCBIWindow pcbi, IStep step, CancellationToken? cancelToken, string initialDir, string filter, int filterIndex, string fileName, string loadedDesignPathOrServer, PCBI.Plugin.Interfaces.FileLocation fileLocation, FileType fileType, string defaultFileName, List<FileFormatFilter> fileFilters)
        {
            if (!pcbi.JobIsLoaded) return "No job is loaded.";
            List<List<PCBI.Automation.IO.Excel.Cell>> cellList = new List<List<PCBI.Automation.IO.Excel.Cell>>();
            List<PCBI.Automation.IO.Excel.Cell> headerRow = new List<PCBI.Automation.IO.Excel.Cell>
