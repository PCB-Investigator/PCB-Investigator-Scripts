...
/// </summary>
    ...
    {
        /// <summary>
        /// Example method to select all via drills in the current design by using the PCB-Investigator API.
        /// </summary>
        private static string Example_SelectViaDrillsInDesign(IPCBIWindow pcbi, IStep step, CancellationToken? cancelToken)
        {
            ...
            // Update the selection and view if any vias were found
            if (count > 0)
            {
                pcbi.UpdateSelection();
                pcbi.UpdateView(NeedFullRedraw: true);
                return "All " + count + " via drills have been selected in the current design.";
            }
            else
            {
                return "There are no via drills in the current design.";
            }
        }
